...
import org.apache.calcite.sql.parser.edshiftParserImpl;
...
/**
 * Unit tests for the PostgreSQL SQL parser.
 */
class PostgreSQLQuidemTest extends QuidemTest {
  /** Runs a test from the command line.
   *
   * <p>For example:
   *
   * <blockquote>
   *   <code>java PostgreSQLQuidemTest sql/table.iq</code>
   * </blockquote> */
  public static void main(String[] args) throws Exception {
    for (String arg : args) {
      new PostgreSQLQuidemTest().test(arg);
    }
  }
  ...
  public static Collection<Object[]> data() {
    ...
    // its files.
    final String first = "sql/postgresql.iq";
    return data(first);
  }
  ...
  @Override protected CommandHandler createCommandHandler() {
    return new PostgreSQLCommandHandler();
  }
  ...
  static class ExplainValidatedCommand extends AbstractCommand {
    ...
    @Override public void execute(Context x, boolean execute) throws Exception {
      if (execute) {
        // use PostgreSQL parser
        final SqlParser.ConfigBuilder parserConfig =
            SqlParser.configBuilder()
                .setParserFactory(PostgreSQLParserImpl.FACTORY);
  ...
  private static class PostgreSQLCommandHandler implements CommandHandler {
