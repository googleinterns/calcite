 ...
 */
final class PostgreSQLParserTest extends PostgreSQLBaseParserTest {
  ...
  @Override protected Tester getTester() {
    return new TesterImpl() {
      @Override protected void checkEx(String expectedMsgPattern,
        ...
        if (thrown != null && thrownByPostgreSQLTest(thrown)) {
      ...
      private boolean thrownByPostgreSQLTest(Throwable ex) {
        Throwable rootCause = Throwables.getRootCause(ex);
        StackTraceElement[] stackTrace = rootCause.getStackTrace();
        for (StackTraceElement stackTraceElement : stackTrace) {
          String className = stackTraceElement.getClassName();
          if (Objects.equals(className, PostgreSQLParserTest.class.getName())) {
            return true;
          }
        }
        return false;
